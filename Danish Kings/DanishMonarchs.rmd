...
author: "Astrid Hannestad and Liv Breinberg Hansen"
date: "2025-03-07"
...
-  `read_csv2()`
```{r}
#Testing how to load the spreadsheet
library(tidyverse)
kings1 <- read.csv("data/Kongeraekken-csv.csv")

kings2 <- read_csv("data/Kongeraekken-csv.csv")

kings3 <- read.csv2("data/Kongeraekken-csv.csv")

kings4 <- read_csv2( "data/Kongeraekken-csv.csv")
...
- it shows that it's a tibble data frame
3. How many columns does the object have when created with these different functions?
- For nr 1 and 2 it shows only one column, because they are made for comma seperated csv files, and for 3 and 4 it shows 5 because they are made for semicolon seperated files, which mine is
...
```{r}
kings %>% 
  summarize(average = mean(reign_duration, na.rm = TRUE))
```
...
kings %>% 
  ...
  slice(1:3) %>% 
  mutate(Days = reign_duration * 365 + floor(reign_duration/4)) %>% 
  select(Monarch, reign_duration, Days)
```
...
It shows that the longer up in time, the longer monarchs live. When the middle ages begin there is a dip in the duration of the reigns, and towards the 1500s and onwards they begin to reign longer and longer, as the monarchy becomes more stable.
...
kings %>%
  ...
  theme_bw()+
   labs(
    title = "Reign duration over time",
    x = "Year",
    y = "Reign duration"
  )

ggsave("kings.png", width = 8, height = 6, dpi = 300)
